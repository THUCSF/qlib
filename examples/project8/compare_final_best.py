...
def plot_daily_strategy(final_results, best_results, prefix):
    xs = list(best_results[0]["daily_return"].keys())
    ys = [np.array(list(dic["daily_return"].values())) for dic in best_results]
    ys = np.stack([np.cumprod(1 + y) for y in ys])
    means = ys.mean(0)
    mins, maxs = ys.min(0), ys.max(0)
    plt.figure(figsize=(30, 10))
    plt.plot(xs, means, label="best")
    plt.fill_between(xs, mins, maxs, alpha=0.2)
    plt.xticks(lastday_month(xs), rotation=30, ha="right")

    ys = [np.array(list(dic["daily_return"].values())) for dic in final_results]
    ys = np.stack([np.cumprod(1 + y) for y in ys])
    means = ys.mean(0)
    mins, maxs = ys.min(0), ys.max(0)
    plt.plot(xs, means, label="final")
    plt.fill_between(xs, mins, maxs, alpha=0.2)
    plt.legend()
        ...
        for data_range, data_dic in loss_dic.items():
            ...
            for stop_strategy in ["final", "best"]:
                ...
                if len(model_repeats) == 0:
                    ...
                    continue
                strategy_results.append(model_repeats)
            save_prefix = f"{args.expr}/{loss_type}_{data_range}"
            plot_daily_strategy(*strategy_results, prefix=save_prefix)
